...
class ARCamView
{
    ...
    {
        // Initializing the original pose created from AlvaARConnectorTHREE
        this.applyPose = AlvaARConnectorTHREE.Initialize( THREE );
        
        // WebGL Render creates beautifully crafted scenes using WebGL.
        ...
        // Camera that uses perspective projection (representing three dimensional scene in two dimensional medium)
        this.camera = new THREE.PerspectiveCamera( 75, width / height, 0.1, 1000 );
        this.camera.rotation.reorder( 'YXZ' );
        this.camera.updateProjectionMatrix();

        // Creates the object to be placed upon detection
        this.object = new THREE.Mesh( new THREE.CylinderGeometry(1, 1, 4, 50), new THREE.MeshNormalMaterial( { flatShading: true } ) );
...
class ARIMUView
{
    constructor( container, width, height, x = 0, y = 0, z = -1, scale = 0.1 )
    {
        ...
        // I don't see this Ring Buffer when running
        ...
        // Perspective Camera: Mimics the way the human eyes see
        this.camera = new THREE.PerspectiveCamera( 75, width / height, 0.1, 1000 );
        this.camera.rotation.reorder( 'YXZ' );
